...
//    recentDocumentsListView->setItemDelegate(m_recentItemDelegate.data());
    ...
    // labelSupportText->setVisible(false);
    // labelNoFeed->setVisible(false);
    widgetRight->setVisible(false);
    hideLayout(horizontalLayout_12);
    hideLayout(horizontalLayout_11);
    hideLayout(horizontalLayout_4);
}

void KisWelcomePageWidget::hideLayout(QBoxLayout* layout) {
    if (!layout) return;
